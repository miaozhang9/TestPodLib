...
//    self.isIconLeft = YES;
    if (self.isIconLeft) {
        [self.userImageView qhMas_makeConstraints:^(QHMASConstraintMaker *make) {
            make.left.equalTo(weakself.contentLayer).offset(25);
            make.centerY.equalTo(weakself.contentLayer).offset(-80);
//            make.size.qhMas_equalTo(weakself.argIcon.size);
            make.size.qhMas_equalTo(CGSizeMake(weakself.argIcon.size.width/2, weakself.argIcon.size.height/2));
        }];
        
        if (self.argIcon) {
            [self.textLabel qhMas_makeConstraints:^(QHMASConstraintMaker *make) {
                make.left.equalTo(weakself.userImageView.qhMas_right).offset(-40);
                make.right.equalTo(weakself.contentLayer).offset(-20);
                make.centerY.equalTo(weakself.contentLayer).offset(-20);
            }];
            
        } else {
            [self.textLabel qhMas_makeConstraints:^(QHMASConstraintMaker *make) {
                make.centerX.equalTo(weakself.contentLayer).offset(0);
                make.centerY.equalTo(weakself.contentLayer).offset(-20);
            }];
...
- (void)qh_addCancelBtnIfPresented{
    if (!self.navigationController) {
        ...
        [btn qhMas_makeConstraints:^(QHMASConstraintMaker *make) {
            make.right.equalTo(weakSelf.transformView).offset(-28);//-14
            make.centerY.equalTo(weakSelf.transformView.qhMas_bottom).offset(-32);//-32
...
- (void)qh_addColorRadius{
    ...
    for (int i=0; i<4; i++) {
        QHFetchCardAlphaRect *r = [[QHFetchCardAlphaRect alloc] initWithRotation:M_PI/2.0*i color:[UIColor qh_colorWithHexString: self.borderColor && self.borderColor.length > 0 ? self.borderColor : @"#cfa767"] lineWidth:2.0];
...
- (UIButton *)cameraBtn{
    if (!_cameraBtn) {
        ...
        [_cameraBtn setImage: self.takePhotoImageB64 && self.takePhotoImageB64.length > 0 ? [UIImage qh_base64ToImage:self.takePhotoImageB64]  :  [QHLoanDoorBundle imageNamed:@"FetchCard_cameraYBtn"] forState:UIControlStateNormal];
         _cameraBtn.transform = CGAffineTransformMakeRotation(-M_PI/2.0);
...
- (UIImageView *)userImageView{
    if (!_userImageView) {
        ...
        _userImageView.transform = CGAffineTransformMakeRotation(-M_PI/2.0);
